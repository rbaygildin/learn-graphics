...
//    }
//
//    facesVao = new GLuint[vCount * hCount];
//
//    for (size_t col = 1; col < hCount; col++) {
//        for (size_t row = 1; row < vCount; row++) {
//            auto vbo = new GLfloat *[4];
//            for (size_t i = 0; i < 4; i++)
//                vbo[i] = new GLfloat[3];
//            for (ULONG coord = 0; coord < 3; coord++) {
//                vbo[0][coord] = vs(row * hCount + col, coord);
//                vbo[1][coord] = vs((row - 1) * hCount + col, coord);
//                vbo[2][coord] = vs((row - 1) * hCount + col - 1, coord);
//                vbo[3][coord] = vs(row * hCount + col - 1, coord);
//            }
//            glGenBuffers(1, &facesVao[row * hCount + col]);
//            glBindBuffer(GL_ARRAY_BUFFER, facesVao[row * hCount + col]);
//            glBufferData(GL_ARRAY_BUFFER, 4 * 3 * sizeof(GLfloat), vbo, GL_STATIC_DRAW);
...
//            vbo[i] = new GLfloat[3];
//        for(ULONG coord = 0; coord < 3; coord++){
//            vbo[0][coord] = vs(row * hCount, coord);
//            vbo[0][coord] = vs((row - 1) * hCount, coord);
//            vbo[0][coord] = vs((row - 1) * hCount + hCount - 1, coord);
//            vbo[0][coord] = vs(row * hCount + hCount - 1, coord);
//        }
//        glGenBuffers(1, &facesVao[row * hCount]);
//        glBindBuffer(GL_ARRAY_BUFFER, facesVao[row * hCount]);
//        glBufferData(GL_ARRAY_BUFFER, 4 * 3 * sizeof(GLfloat), vbo, GL_STATIC_DRAW);
//        glBindBuffer(GL_ARRAY_BUFFER, 0);
//    }
}
