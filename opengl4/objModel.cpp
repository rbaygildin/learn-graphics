...
string GetDirectoryPath(const string &sFilePath) {
    // Get directory path
//    RFOR(i, ESZ(sFilePath) - 1) if (sFilePath[i] == '/') {
//            sDirectory = sFilePath.substr(0, i + 1);
//            break;
//        }
    return boost::filesystem::path(sFilePath).parent_path().string();
}
...
bool CObjModel::LoadModel(string sFileName, string sMtlFileName) {
    ...
    if (iAttrBitField & 1) {
        ...
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, iDataStride, nullptr);
    }
    // Texture coordinates
    if (iAttrBitField & 2) {
...
bool CObjModel::LoadMaterial(string sFullMtlFileName) {
    ...
    while (fgets(line, 255, fp)) {
        ...
        if (sType == "map_Ka") {
            ...
            sTextureName.erase(remove_if(sTextureName.begin(), sTextureName.end(), [](unsigned char c) {
                return c == '\r' || c == '\n';
            }), sTextureName.end());
