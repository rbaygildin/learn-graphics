...
void
Texture::CreateFromData(BYTE *bData, int width, int height, int a_iBPP, GLenum format, bool bGenerateMipMaps) {
...
// a_sPath - path to the texture
// bGenerateMipMaps - whether to create mipmaps
bool Texture::LoadTexture2D(string path, bool bGenerateMipMaps) {
    ...
    if (bDataPointer == nullptr || FreeImage_GetWidth(dib) == 0 || FreeImage_GetHeight(dib) == 0)
        return false;
    ...
    if (FreeImage_GetBPP(dib) == 8)
        format = GL_LUMINANCE;
    CreateFromData(bDataPointer, FreeImage_GetWidth(dib), FreeImage_GetHeight(dib), FreeImage_GetBPP(dib), format,
                   bGenerateMipMaps);
